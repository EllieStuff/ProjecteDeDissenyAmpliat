...
public class TrajectoryCalculator : MonoBehaviour
{
    ...
    public void CalculateTrajectory(Vector2 _initPos, Vector2 _initForce, Vector2 _moveDir)
    {
        for(int i = 0; i < trajectoryPoints.Length; i++)
        {
            ...
            if (_moveDir != Vector2.zero)
            {
                ...
                Vector3 finalPos = new Vector2(
                    ...
                    _initPos.y + (_initForce.y * _moveDir.y * currTimeDiff) + (0.5f * GRAVITY * Mathf.Pow(currTimeDiff, 2))
                );
